...
export abstract class Project implements LanguageServiceHost, ModuleResolutionHost {
  ...
  getCompileOnSaveAffectedFileList(scriptInfo: ScriptInfo): string[] {
    ...
    return mapDefined(
      ...
      (sourceFile) =>
        this.shouldEmitFile(this.projectService.getScriptInfoForPath(sourceFile.path))
          ? sourceFile.fileName
          : undefined
    )
  }
  ...
  emitFile(
    ...
    const { emitSkipped, diagnostics, outputFiles } = this.getLanguageService().getEmitOutput(
      scriptInfo.fileName
    )
    if (!emitSkipped) {
    ...
    return this.getFileNames().map(
      (fileName): protocol.FileWithProjectReferenceRedirectInfo => ({
        ...
        isSourceOfProjectReferenceRedirect:
          includeProjectReferenceRedirectInfo && this.isSourceOfProjectReferenceRedirect(fileName),
      })
    )
  }
  ...
  updateGraph(): boolean {
    ...
    const changedFiles: readonly Path[] =
      this.resolutionCache.finishRecordingFilesWithChangedResolutions() || emptyArray
  ...
  print(writeProjectFileNames: boolean) {
    ...
    this.writeLog(
      this.filesToString(writeProjectFileNames && this.projectService.logger.hasLevel(LogLevel.verbose))
    )
    this.writeLog('-----------------------------------------------')
    if (this.autoImportProviderHost) {
  ...
  beginEnablePluginSync(
    ...
    const resolvedModule = firstDefined(
      searchPaths,
      (searchPath) =>
        Project.resolveModule(pluginConfigEntry.name, searchPath, this.projectService.host, log, logError) as
          | PluginModuleFactory
          | undefined
    )
    ...
    if (rootNames?.length) {
      hostProject.log(
        `AutoImportProviderProject: found ${
          rootNames.length
        } root files in ${dependenciesAdded} dependencies in ${timestamp() - start} ms`
      )
    }
...
export class ConfiguredProject extends Project {
  ...
  override getParsedCommandLine(fileName: string) {
    ...
    let configFileExistenceInfo =
      this.projectService.configFileExistenceInfoCache.get(canonicalConfigFilePath)
  ...
  setPotentialProjectReference(canonicalConfigPath: NormalizedPath) {
    ...
    ;(this.potentialProjectReferences || (this.potentialProjectReferences = new Set())).add(
      canonicalConfigPath
    )
  }
  ...
  hasOpenRef() {
    ...
    return (
      (!!configFileExistenceInfo.openFilesImpactedByConfigFile &&
        forEachEntry(configFileExistenceInfo.openFilesImpactedByConfigFile, (_value, infoPath) => {
