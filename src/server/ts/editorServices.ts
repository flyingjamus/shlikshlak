...
import { ITypingsInstaller, nullTypingsInstaller, TypingsCache } from './typingsCache'
import { getBaseConfigFileName, ThrottledOperations } from './utilities'
import {
...
export class ProjectService {
  ...
  /** @internal */
  readonly newAutoImportProviderProjectName = createProjectNameFactoryWithCounter(
    makeAutoImportProviderProjectName
  )
  /** @internal */
  ...
  constructor(opts: ProjectServiceOptions) {
    ...
    this.typesMapLocation =
      opts.typesMapLocation === undefined
        ? combinePaths(getDirectoryPath(this.getExecutingFilePath()), 'typesMap.json')
          ...
          if (this.openFiles.has(fileOrDirectoryPath)) {
            ...
            if (info.isAttached(project)) {
              const loadLevelToSet = Math.max(
                reloadLevel,
                project.openFileWatchTriggered.get(fileOrDirectoryPath) || ConfigFileProgramReloadLevel.None
  ...
  private stopWatchingConfigFilesForClosedScriptInfo(info: ScriptInfo) {
    ...
    this.forEachConfigFileLocation(info, (canonicalConfigFilePath) => {
      ...
      if (configFileExistenceInfo) {
        ...
        if (!configFileExistenceInfo.openFilesImpactedByConfigFile?.size && !configFileExistenceInfo.config) {
  ...
  logErrorForScriptInfoNotFound(fileName: string): void {
    const names = arrayFrom(this.filenameToScriptInfo.entries(), ([path, scriptInfo]) => ({
      path,
  ...
  setHostConfiguration(args: protocol.ConfigureRequestArguments) {
    if (args.file) {
      ...
      if (args.preferences) {
        const { lazyConfiguredProjectsFromExternalProject, includePackageJsonAutoImports } =
  ...
  getOriginalLocationEnsuringConfiguredProject(
    ...
    if (!configuredProject) {
      if (project.getCompilerOptions().disableReferencedProjectLoad) {
        ...
        return scriptInfo?.containingProjects.length ? originalLocation : location
      }
    ...
    if (configuredProject.isSolution() || !projectContainsOriginalInfo(configuredProject)) {
      ...
      configuredProject = forEachResolvedProjectReferenceProject(
        ...
        } to find possible configured project for original file: ${originalFileInfo.fileName}${
          location !== originalLocation ? ' for location: ' + location.fileName : ''
        }`
      )
  ...
  private createAncestorProjects(info: ScriptInfo, project: ConfiguredProject) {
    ...
    while (true) {
      ...
      if (
        !project.isInitialLoadPending() &&
        (!project.getCompilerOptions().composite || project.getCompilerOptions().disableSolutionSearching)
      )
        return
  ...
  private ensureProjectChildren(
    ...
    for (const child of children) {
      if (!child) continue
      const referencedProject = forEachResolvedProjectReference(child.references, (ref) =>
  ...
  requestEnablePlugin(
    ...
    project.endEnablePlugin(
      project.beginEnablePluginSync(pluginConfigEntry, searchPaths, pluginConfigOverrides)
    )
  }
    ...
    return forEachAncestorDirectory(fileName, (directory) => {
      switch (this.packageJsonCache.directoryHasPackageJson(this.toPath(directory))) {
        ...
        case Ternary.False:
          return undefined
        case Ternary.Maybe:
          return this.host.fileExists(combinePaths(directory, 'package.json')) ? directory : undefined
      }
    })
  }
