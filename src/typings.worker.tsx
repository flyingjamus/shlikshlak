...
// await import(resources.typescript)
// self.importScripts()

const ROOT_URL = `https://cdn.jsdelivr.net/`

const store = createStore('db', 'typescript-definitions-cache-v1')
...
const fetchAsText = (url: string): Promise<string> => {
  ...
  const promise = fetch(url).then((response) => {
    ...
    throw new Error(`Failed to fetch typings (${response.statusText || response.status}) for: ${url}`)
  })
...
const fetchFromDefinitelyTyped = (dependency: string, _version: string, output: FetchOutput) =>
  fetchAsText(`${ROOT_URL}npm/@types/${dependency.replace('@', '').replace(/\//g, '__')}/index.d.ts`).then(
    (typings: string) => {
...
const getRequireStatements = (title: string, code: string) => {
  ...
  const sourceFile = ts.createSourceFile(title, code, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS)
...
const getFileMetaData = (dependency: string, version: string, depPath: string): Promise<FileMetadata> =>
  ...
  return fetchAsText(`${depUrl}/package.json`)
    ...
    .then((packageJSON: { typings?: string; types?: string; version?: string }) => {
      ...
      if (types) {
        ...
        return getFileMetaData(dependency, resolvedVersion, path.join('/', path.dirname(types))).then(
          (fileMetaData: FileMetadata) => {
