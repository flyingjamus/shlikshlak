...
export function installHook(target: any): DevToolsHook | null {
  ...
  function dangerous_setTargetConsoleForTesting(targetConsoleForTesting: Record<string, any>): void {
  ...
  function checkDCE(fn: (...args: Array<any>) => any) {
  ...
  function formatWithStyles(inputArgs: ReadonlyArray<any>, style?: string): ReadonlyArray<any> {
    if (
      inputArgs === undefined ||
      inputArgs === null ||
  ...
  function getInternalModuleRanges(): Array<[StackFrameString, StackFrameString]> {
    return moduleRanges
  }
