...
import type { HookNames, HookSourceLocationKey } from './types'
import type { HookSource } from '../../react-debug-tools/src/ReactDebugHooks'
import type { FetchFileWithCaching } from './devtools/views/Components/FetchFileWithCachingContext'
      ...
      (done) => {
        loadHookNamesFunction(hooksTree, fetchFileWithCaching).then(
          function onSuccess(hookNames) {
            ...
            if (hookNames) {
              const resolvedRecord = newRecord as any as ResolvedRecord<HookNames>
