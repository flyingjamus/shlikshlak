import type { Lane, Lanes, DevToolsProfilingHooks, WorkTagMap, CurrentDispatcherRef } from './types'
...
export function setPerformanceMock_ONLY_FOR_TESTING(performanceMock: Performance | null) {
...
export function createProfilingHooks({
  ...
  function recordReactMeasureStarted(type: ReactMeasureType, lanes: Lanes): void {
  ...
  function markComponentErrored(fiber: Fiber, thrownValue: unknown, lanes: Lanes): void {
    if (isProfiling || supportsUserTimingV3) {
      const componentName = getDisplayNameForFiber(fiber) || 'Unknown'
  ...
  function markComponentSuspended(fiber: Fiber, wakeable: Wakeable, lanes: Lanes): void {
    if (isProfiling || supportsUserTimingV3) {
