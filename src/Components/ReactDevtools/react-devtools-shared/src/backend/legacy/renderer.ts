...
import { deletePathInObject, getDisplayName, getInObject, renamePathInObject, setInObject } from '../../utils'
import {
  __DEBUG__,
  TREE_OPERATION_ADD,
...
export type InternalInstance = Record<string, any>
type LegacyRenderer = Record<string, any>
...
function getChildren(internalInstance: Record<string, any>): Array<any> {
  const children = []
...
export function attach(
  ...
  const internalInstanceToIDMap: WeakMap<InternalInstance, number> = new WeakMap()
  const internalInstanceToRootIDMap: WeakMap<InternalInstance, number> = new WeakMap()
  let getInternalIDForNative: GetFiberIDForNative = null as any as GetFiberIDForNative
  let findNativeNodeForInternalID: (id: number) => NativeType | null | undefined
  ...
  function recordMount(internalInstance: InternalInstance, id: number, parentID: number) {
  ...
  function crawlAndRecordInitialMounts(id: number, parentID: number, rootID: number) {
    if (__DEBUG__) {
    ...
    if (internalInstance != null) {
      ...
      if (publicInstance != null) {
        switch (type) {
          ...
          case 'props':
            const element = internalInstance._currentElement
            internalInstance._currentElement = { ...element, props: copyWithDelete(element.props, path) }
            forceUpdate(publicInstance)
            break
    ...
    if (internalInstance != null) {
      ...
      if (publicInstance != null) {
        switch (type) {
          ...
          case 'props':
            const element = internalInstance._currentElement
            internalInstance._currentElement = { ...element, props: copyWithSet(element.props, path, value) }
            forceUpdate(publicInstance)
            break
