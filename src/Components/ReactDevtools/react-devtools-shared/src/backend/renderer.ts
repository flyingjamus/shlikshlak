...
export function attach(
  ...
  function updateMostRecentlyInspectedElementIfNecessary(fiberID: number): void {
  ...
  function onErrorOrWarning(fiber: Fiber, type: 'error' | 'warn', args: ReadonlyArray<any>): void {
    if (type === 'error') {
  ...
  function getOrGenerateFiberID(fiber: Fiber): number {
    ...
    const refinedID = id as any as number
  ...
  function getFiberIDUnsafe(fiber: Fiber): number | null {
    if (fiberToIDMap.has(fiber)) {
      return fiberToIDMap.get(fiber) as any as number
    } else {
      const { alternate } = fiber

      if (alternate !== null && fiberToIDMap.has(alternate)) {
        return fiberToIDMap.get(alternate) as any as number
      }
    }
  ...
  function getChangeDescription(prevFiber: Fiber | null, nextFiber: Fiber): ChangeDescription | null {
  ...
  function getContextsForFiber(fiber: Fiber): [Record<string, any>, any] | null {
  ...
  function flushPendingErrorsAndWarningsAfterDelay() {
    ...
    flushPendingErrorsAndWarningsAfterDelayTimeoutID = setTimeout(() => {
      ...
      const operations: OperationsArray = new Array(3 + pendingOperations.length)
  ...
  function mergeMapsAndGetCountHelper(
    ...
    if (pendingMessageCountMap != null) {
      if (messageCountMap == null) {
        ...
        const refinedMessageCountMap = messageCountMap as any as Map<string, number>
  ...
  function flushPendingEvents(root: Record<string, any>): void {
  ...
  function recordMount(fiber: Fiber, parentFiber: Fiber | null) {
    ...
    if (isRoot) {
      ...
      if (
        ...
        ((parentFiber as any as Fiber).mode & StrictModeBits) === 0
      ) {
  ...
  function recordUnmount(fiber: Fiber, isSimulated: boolean) {
    ...
    // Flow refinement.
    const id = unsafeID as any as number
    const isRoot = fiber.tag === HostRoot
  ...
  function findReorderedChildrenRecursively(fiber: Fiber, nextChildren: Array<number>) {
    if (!shouldFilterFiber(fiber)) {
    ...
    if (isProfiling && isProfilingSupported) {
      if (!shouldBailoutWithPendingOperations()) {
        ...
        if (commitProfilingMetadata != null) {
          ...
          ;(rootToCommitProfilingMetadataMap as any as CommitProfilingMetadataMap).set(currentRootID, [
  ...
  function getDisplayNameForFiberID(id) {
    const fiber = idToArbitraryFiberMap.get(id)
    return fiber != null ? getDisplayNameForFiber(fiber as any as Fiber) : null
  }
  ...
  function getFiberIDForNative(hostInstance, findNearestUnfilteredAncestor = false) {
    ...
    if (fiber != null) {
      ...
      return getFiberIDThrows(fiber as any as Fiber)
    }
  ...
  // END copied code
  function prepareViewAttributeSource(id: number, path: Array<string | number>): void {
    if (isMostRecentlyInspectedElement(id)) {
      window.$attribute = getInObject(mostRecentlyInspectedElement as any as InspectedElement, path)
    }
  }
  ...
  function createIsPathAllowed(key: string | null, secondaryCategory: 'hooks' | null) {
  ...
  function storeAsGlobal(id: number, path: Array<string | number>, count: number): void {
    if (isMostRecentlyInspectedElement(id)) {
      const value = getInObject(mostRecentlyInspectedElement as any as InspectedElement, path)
  ...
  function copyElementPath(id: number, path: Array<string | number>): void {
    if (isMostRecentlyInspectedElement(id)) {
      copyToClipboard(getInObject(mostRecentlyInspectedElement as any as InspectedElement, path))
    }
  }
        ...
        if (path !== null) {
          ...
          return {
            ...
            value: cleanForBridge(
              getInObject(mostRecentlyInspectedElement as any as InspectedElement, path),
      ...
      switch (type) {
        ...
        case 'hooks':
          if (typeof overrideHookStateRenamePath === 'function') {
            overrideHookStateRenamePath(fiber, hookID as any as number, oldPath, newPath)
          }
  ...
  function startProfiling(shouldRecordChangeDescriptions: boolean) {
    ...
    hook.getFiberRoots(rendererID).forEach((root) => {
      ...
      ;(displayNamesByRootID as any as DisplayNamesByRootID).set(rootID, getDisplayNameForRoot(root.current))
  ...
  function shouldSuspendFiberAccordingToSet(fiber) {
    const maybeID = getFiberIDUnsafe(fiber as any as Fiber)
  ...
  const formatPriorityLevel = (priorityLevel: number | null | undefined) => {
