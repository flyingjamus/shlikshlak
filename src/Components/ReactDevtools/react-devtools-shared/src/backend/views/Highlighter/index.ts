...
export default function setupHighlighter(bridge: BackendBridge, agent: Agent): void {
  ...
  function highlightNativeElement({
    ...
    let nodes: Array<HTMLElement> | null | undefined = null

    if (renderer != null) {
      nodes = renderer.findNativeNodesForFiberID(id) as any as Array<HTMLElement> | null | undefined
    }
  ...
  function onPointerDown(event: MouseEvent) {
    event.preventDefault()
    event.stopPropagation()
    selectFiberForNode(event.target as any as HTMLElement)
  }

  function onPointerOver(event: MouseEvent) {
    event.preventDefault()
    event.stopPropagation()
    const target = event.target as any as HTMLElement
