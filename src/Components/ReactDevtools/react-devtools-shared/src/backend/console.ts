...
const consoleManagedByDevToolsDuringStrictMode = false
...
function getConsoleColor(method: string): string | null | undefined {
  switch (method) {
    case 'warn':
...
type OnErrorOrWarning = (fiber: Fiber, type: 'error' | 'warn', args: Array<any>) => void
const injectedRenderers: Map<
  ReactRenderer,
  {
    currentDispatcherRef: CurrentDispatcherRef
    getCurrentFiber: () => Fiber | null
    onErrorOrWarning: OnErrorOrWarning | null | undefined
    workTagMap: WorkTagMap
  }
> = new Map()
let targetConsole: Record<string, any> = console
...
export function dangerous_setTargetConsoleForTesting(targetConsoleForTesting: Record<string, any>): void {
...
export function registerRenderer(renderer: ReactRenderer, onErrorOrWarning?: OnErrorOrWarning): void {
      ...
      try {
        ...
        const overrideMethod = (...args) => {
          ...
          for (const {
            ...
            const current: Fiber | null | undefined = getCurrentFiber()
              ...
              try {
                if (shouldShowInlineWarningsAndErrors) {
                  ...
                  if (typeof onErrorOrWarning === 'function') {
                    onErrorOrWarning(
                      current,
                      method as any as 'error' | 'warn', // Copy args before we mutate them (e.g. adding the component stack)
                      args.slice()
                    )
                  }
                }
