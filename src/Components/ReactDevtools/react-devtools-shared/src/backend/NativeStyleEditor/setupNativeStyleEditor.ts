...
export type ResolveNativeStyle = (stylesheetID: any) => Record<string, any> | null | undefined
export default function setupNativeStyleEditor(
  ...
  bridge.addListener(
    ...
    ({ id, rendererID, name, value }: { id: number; rendererID: number; name: string; value: string }) => {
...
const componentIDToStyleOverrides: Map<number, Record<string, any>> = new Map()
...
function shallowClone(object: Record<string, any>): Record<string, any> {
  const cloned = {}
...
function setStyle(agent: Agent, id: number, rendererID: RendererID, name: string, value: string) {
  const data = agent.getInstanceAndStyle({
    id,
    rendererID,
  })
