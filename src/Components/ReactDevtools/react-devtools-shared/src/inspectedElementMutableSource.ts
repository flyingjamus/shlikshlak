...
import { fillInPath } from './hydration'
...
const inspectedElementCache: LRUCache<number, InspectedElementFrontend> = new LRU({
  max: 25,
})
type Path = Array<string | number>
type InspectElementReturnType = [InspectedElementFrontend, InspectedElementResponseType]
    ...
    switch (type) {
      ...
      case 'hydrated-path':
        ...
        if (inspectedElement !== null) {
          ...
          // Merge hydrated data
          fillInPath(inspectedElement, value, path as any as Path, hydrateHelper(value, path as any as Path))
          inspectedElementCache.set(id, inspectedElement)
          return [inspectedElement, type]
        }
      ...
      default:
        // Should never happen.
        if (__DEV__) {
          console.error(`Unexpected inspected element response data: "${type}"`)
        }
