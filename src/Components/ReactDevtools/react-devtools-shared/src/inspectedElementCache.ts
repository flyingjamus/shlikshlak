import { unstable_getCacheForType as getCacheForType, startTransition } from "react";
...
import type { Element, InspectedElement as InspectedElementFrontend, InspectedElementResponseType } from "react-devtools-shared/src/devtools/views/Components/types";
const Pending = 0;
const Resolved = 1;
const Rejected = 2;
type PendingRecord = {
 ...
 */
export function inspectElement(element: Element, path: Array<string | number> | null, store: Store, bridge: FrontendBridge): InspectedElementFrontend | null {
  ...
  if (!record) {
    ...
    if (rendererID == null) {
      const rejectedRecord = ((newRecord as any) as RejectedRecord);
    ...
    inspectElementMutableSource({
      ...
      element,
      path,
      rendererID: ((rendererID as any) as number)
    }).then(([inspectedElement]) => {
      const resolvedRecord = ((newRecord as any) as ResolvedRecord<InspectedElementFrontend>);
...
type RefreshFunction = (seedKey: CacheSeedKey, cacheMap: InspectedElementMap) => void;
