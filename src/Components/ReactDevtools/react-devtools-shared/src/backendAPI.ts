...
import { hydrate, fillInPath } from './hydration'
...
import type { BackendEvents, FrontendBridge } from './bridge'
import type {
  DehydratedData,
  InspectedElement as InspectedElementFrontend,
} from '../../react-devtools-shared/src/devtools/views/Components/types'
export function clearErrorsAndWarnings({ bridge, store }: { bridge: FrontendBridge; store: Store }): void {
  ...
  const inspectedElement: InspectedElementFrontend = {
    ...
    owners:
      owners === null
        ? null
        : owners.map((owner) => {
            const [displayName, hocDisplayNames] = separateDisplayNameAndHOCs(owner.displayName, owner.type)
            return { ...owner, displayName, hocDisplayNames }
          }),
    ...
    if (path) {
      ...
      if (length > 0) {
        ...
        return hydrate(
          ...
          unserializable.map((unserializablePath) => unserializablePath.slice(length))
        )
      }
    }
