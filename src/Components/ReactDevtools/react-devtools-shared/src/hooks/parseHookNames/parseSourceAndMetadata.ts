...
import { withAsyncPerfMeasurements, withSyncPerfMeasurements } from "react-devtools-shared/src/PerformanceLoggingUtils";
import SourceMapConsumer from "../SourceMapConsumer";
import type { SourceMapConsumerType } from "../SourceMapConsumer";
import type { HooksList, LocationKeyToHookSourceAndMetadata } from "./loadSourceAndMetadata";
...
const runtimeURLToMetadataCache: LRUCache<string, CachedRuntimeCodeMetadata> = new LRU({
  max: 50
});
...
const originalURLToMetadataCache: LRUCache<string, CachedSourceCodeMetadata> = new LRU({
  max: 50,
...
export async function parseSourceAndMetadata(hooksList: HooksList, locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata): Promise<HookNames | null> {
...
function findHookNames(hooksList: HooksList, locationKeyToHookParsedMetadata: LocationKeyToHookParsedMetadata): HookNames {
  ...
  hooksList.map(hook => {
    ...
    if (name == null) {
      name = withSyncPerfMeasurements('getHookName()', () => getHookName(hook, hookParsedMetadata.originalSourceAST, ((hookParsedMetadata.originalSourceCode as any) as string), ((originalSourceLineNumber as any) as number), originalSourceColumnNumber));
    }
...
function initializeHookParsedMetadata(locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata) {
...
function parseSourceAST(locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata, locationKeyToHookParsedMetadata: LocationKeyToHookParsedMetadata): void {
  locationKeyToHookSourceAndMetadata.forEach((hookSourceAndMetadata, locationKey) => {
    const hookParsedMetadata = locationKeyToHookParsedMetadata.get(locationKey);
    ...
    const runtimeSourceCode = ((hookSourceAndMetadata.runtimeSourceCode as any) as string);
    ...
    if (sourceMetadata != null) {
      ...
      try {
        ...
        const originalSourceAST = withSyncPerfMeasurements('[@babel/parser] parse(originalSourceCode)', () => parse(originalSourceCode, {
...
function parseSourceMaps(locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata, locationKeyToHookParsedMetadata: LocationKeyToHookParsedMetadata) {
  locationKeyToHookSourceAndMetadata.forEach((hookSourceAndMetadata, locationKey) => {
    const hookParsedMetadata = locationKeyToHookParsedMetadata.get(locationKey);
    ...
    if (runtimeMetadata != null) {
      ...
      if (sourceMapJSON != null) {
        ...
        const metadataConsumer = withSyncPerfMeasurements('new SourceMapMetadataConsumer(sourceMapJSON)', () => new SourceMapMetadataConsumer(sourceMapJSON));
