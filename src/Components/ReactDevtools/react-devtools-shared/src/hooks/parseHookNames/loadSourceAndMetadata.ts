...
import { withAsyncPerfMeasurements, withCallbackPerfMeasurements, withSyncPerfMeasurements } from "react-devtools-shared/src/PerformanceLoggingUtils";
...
export async function loadSourceAndMetadata(hooksList: HooksList, fetchFileWithCaching: FetchFileWithCaching | null): Promise<LocationKeyToHookSourceAndMetadata> {
  return withAsyncPerfMeasurements('loadSourceAndMetadata()', async () => {
    const locationKeyToHookSourceAndMetadata = withSyncPerfMeasurements('initializeHookSourceAndMetadata', () => initializeHookSourceAndMetadata(hooksList));
    await withAsyncPerfMeasurements('loadSourceFiles()', () => loadSourceFiles(locationKeyToHookSourceAndMetadata, fetchFileWithCaching));
    await withAsyncPerfMeasurements('extractAndLoadSourceMapJSON()', () => extractAndLoadSourceMapJSON(locationKeyToHookSourceAndMetadata));
...
function decodeBase64String(encoded: string): Record<string, any> {
...
function extractAndLoadSourceMapJSON(locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata): Promise<any> {
  ...
  locationKeyToHookSourceAndMetadata.forEach(hookSourceAndMetadata => {
    ...
    let sourceMappingURLMatch = withSyncPerfMeasurements('sourceMapRegex.exec(runtimeSourceCode)', () => sourceMapRegex.exec(runtimeSourceCode));
      ...
      while (sourceMappingURLMatch != null) {
        ...
        if (hasInlineSourceMap) {
          try {
            ...
            const trimmed = ((sourceMappingURL.match(/base64,([a-zA-Z0-9+\/=]+)/) as any) as Array<string>)[1];
            const decoded = withSyncPerfMeasurements('decodeBase64String()', () => decodeBase64String(trimmed));
            const sourceMapJSON = withSyncPerfMeasurements('JSON.parse(decoded)', () => JSON.parse(decoded));
        ...
        sourceMappingURLMatch = withSyncPerfMeasurements('sourceMapRegex.exec(runtimeSourceCode)', () => sourceMapRegex.exec(runtimeSourceCode));
      }
      ...
      if (hookSourceAndMetadata.sourceMapJSON === null) {
        externalSourceMapURLs.forEach((sourceMappingURL, index) => {
          ...
          const fetchPromise = dedupedFetchPromises.get(url) || fetchFile(url).then(sourceMapContents => {
            const sourceMapJSON = withSyncPerfMeasurements('JSON.parse(sourceMapContents)', () => JSON.parse(sourceMapContents));
...
function initializeHookSourceAndMetadata(hooksList: Array<HooksNode>): LocationKeyToHookSourceAndMetadata {
...
function loadSourceFiles(locationKeyToHookSourceAndMetadata: LocationKeyToHookSourceAndMetadata, fetchFileWithCaching: FetchFileWithCaching | null): Promise<any> {
  ...
  locationKeyToHookSourceAndMetadata.forEach(hookSourceAndMetadata => {
    ...
    if (fetchFileWithCaching != null) {
      ...
      fetchFileFunction = url => {
        return withAsyncPerfMeasurements(`fetchFileWithCaching("${url}")`, () => {
