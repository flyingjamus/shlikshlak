...
import type { IndexSourceMap, IndexSourceMapSection, BasicSourceMap, MixedSourceMap } from "./SourceMapTypes";
type SearchPosition = {
  columnNumber: number;
  lineNumber: number;
};
...
export type SourceMapConsumerType = {
  originalPositionFor: (arg0: SearchPosition) => ResultPosition;
};
type Mappings = Array<Array<Array<number>>>;
export default function SourceMapConsumer(sourceMapJSON: MixedSourceMap | IndexSourceMapSection): SourceMapConsumerType {
...
function BasicSourceMapConsumer(sourceMapJSON: BasicSourceMap) {
  const decodedMappings: Mappings = withSyncPerfMeasurements('Decoding source map mappings with sourcemap-codec', () => decode(sourceMapJSON.mappings));
  ...
  function originalPositionFor({
    ...
    return {
      column,
      line,
      sourceContent: ((sourceContent as any) as string),
      sourceURL: ((sourceURL as any) as string)
    };
  }
