...
import type { ReactSourceMetadata, IndexSourceMap, BasicSourceMap, MixedSourceMap } from "./SourceMapTypes";
...
type MetadataMap = Map<string, ReactSourceMetadata | null | undefined>;
...
export class SourceMapMetadataConsumer {
  ...
  _metadataBySource: MetadataMap | null | undefined;
  ...
  hookNameFor({
    ...
    return getHookNameForLocation({
      line,
      column
    }, hookMap);
  }

  hasHookMap(source: string | null | undefined): boolean {
    if (source == null) {
      return false;
    }
  ...
  _getMetadataObjectsBySourceNames(sourcemap: MixedSourceMap): MetadataMap {
    ...
    if (sourcemap.hasOwnProperty(REACT_SOURCES_EXTENSION_KEY) && sourcemap[REACT_SOURCES_EXTENSION_KEY] != null) {
      const reactMetadataArray = sourcemap[REACT_SOURCES_EXTENSION_KEY];
      reactMetadataArray.filter(Boolean).forEach(updateMap);
    } else if (sourcemap.hasOwnProperty(FB_SOURCES_EXTENSION_KEY) && sourcemap[FB_SOURCES_EXTENSION_KEY] != null) {
      ...
      if (fbMetadataArray != null) {
        fbMetadataArray.forEach((fbMetadata, sourceIndex) => {
          ...
          const reactMetadata = fbMetadata != null ? fbMetadata[REACT_METADATA_INDEX_IN_FB_METADATA] : null;
   ...
   */
  _getHookMapForSource(source: string): HookMap | null | undefined {
