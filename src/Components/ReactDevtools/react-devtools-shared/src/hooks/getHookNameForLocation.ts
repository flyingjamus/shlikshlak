import type { HookMap, HookMapEntry, HookMapLine, HookMapMappings } from "./generateHookMap";
 ...
 */
export function getHookNameForLocation(location: Position, hookMap: HookMap): string | null {
  const {
    names,
    mappings
  } = hookMap;
...
function binSearch<T>(location: Position, items: T[], compare: (location: Position, items: T[], index: number) => {
  index: number | null;
  direction: number;
}): T | null {
 ...
 */
function compareLinePositions(location: Position, mappings: HookMapMappings, index: number): {
  index: number | null;
  direction: number;
} {
 ...
 * next step in the binary search.
 */
function compareColumnPositions(location: Position, line: HookMapLine, index: number): {
  index: number | null;
  direction: number;
} {
  const startIndex = index;
