...
function getFilteredHookASTNodes(potentialReactHookASTNode: NodePath, potentialHooksFound: Array<NodePath>, source: string): Array<NodePath> {
...
// Returns Hook name
export function getHookName(hook: HooksNode, originalSourceAST: unknown, originalSourceCode: string, originalSourceLineNumber: number, originalSourceColumnNumber: number): string | null {
...
function getHookNameFromNode(originalHook: HooksNode, nodesAssociatedWithReactHookASTNode: NodePath[], potentialReactHookASTNode: NodePath): string | null {
...
function getPotentialHookDeclarationsFromAST(sourceAST: File): NodePath[] {
  ...
  withSyncPerfMeasurements('traverse(sourceAST)', () => traverse(sourceAST, {
    enter(path) {
 ...
 */
export function getHookNamesMappingFromAST(sourceAST: File): ReadonlyArray<{
  name: string;
  start: Position;
}> {
  const hookStack = [];
    ...
    [AST_NODE_TYPES.VARIABLE_DECLARATOR]: {
      enter(path) {
        ...
        if (isConfirmedHookDeclaration(path)) {
          ...
          const referencePaths = hookDeclaredVariableName != null ? path.scope.bindings[hookDeclaredVariableName]?.referencePaths : null;
