...
import { ElementTypeClass, ElementTypeForwardRef, ElementTypeFunction, ElementTypeMemo } from './types'
import { localStorageGetItem, localStorageSetItem } from './storage'
import { meta } from './hydration'
import isArray from './isArray'
import type { ComponentFilter, ElementType } from './types'
import type { LRUCache } from './types'
...
export function getAllEnumerableKeys(obj: Record<string, any>): Set<string | number | Symbol> {
  const keys = new Set()
  let current = obj
...
export function setAppendComponentStack(value: boolean): void {
  localStorageSetItem(LOCAL_STORAGE_SHOULD_APPEND_COMPONENT_STACK_KEY, JSON.stringify(value))
}
...
export function sethideConsoleLogsInStrictMode(value: boolean): void {
  localStorageSetItem(LOCAL_STORAGE_HIDE_CONSOLE_LOGS_IN_STRICT_MODE, JSON.stringify(value))
}
...
export function shallowDiffers(prev: Record<string, any>, next: Record<string, any>): boolean {
...
export function getInObject(object: Record<string, any>, path: Array<string | number>): any {
  return path.reduce((reduced: Record<string, any>, attr: any): any => {
    if (reduced) {
...
export function deletePathInObject(object: Record<string, any>, path: Array<string | number>) {
...
export function setInObject(object: Record<string, any>, path: Array<string | number>, value: any) {
  const length = path.length
  const last = path[length - 1]
 ...
 */
export function getDataType(data: Record<string, any>): DataType {
  if (data === null) {
    return 'null'
...
export function getDisplayNameForReactElement(
  element: React.ReactElement<React.ComponentProps<any>, any>
): string | null {
...
function truncateForDisplay(string: string, length: number = MAX_PREVIEW_STRING_LENGTH) {
...
// Would show a preview of...
//   [123, "abc", Array(2), {…}]
export function formatDataForPreview(data: any, showFormattedValue: boolean): string {
