...
export const currentBridgeProtocol: BridgeProtocol = BRIDGE_PROTOCOL[BRIDGE_PROTOCOL.length - 1]
...
type OverrideHookState = OverrideValue & {
  hookID: number
}
...
type OverrideError = ElementAndRendererID & {
  forceError: boolean
}
type OverrideSuspense = ElementAndRendererID & {
...
> extends EventEmitter<IncomingEvents & OutgoingEvents> {
  ...
  _wall: Wall
  _wallUnlisten: ((...args: Array<any>) => any) | null = null
  ...
  send<EventName extends $Keys<OutgoingEvents>>(
    event: EventName,
  ...
  overrideValueAtPath = ({ id, path, rendererID, type, value }: OverrideValueAtPath) => {
    switch (type) {
      case 'context':
