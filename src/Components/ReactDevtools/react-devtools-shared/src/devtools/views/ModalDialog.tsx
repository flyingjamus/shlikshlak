...
type ModalDialogContextType = State & {
  dispatch: Dispatch;
};
const ModalDialogContext = createContext<ModalDialogContextType>(((null as any) as ModalDialogContextType));
...
function ModalDialogContextController({
  ...
  return <ModalDialogContext.Provider value={value}>
      {children}
...
function ModalDialog(_: {}) {
  ...
  return <div className={styles.Background}>
      {dialogs.map(dialog => <ModalDialogImpl key={dialog.id} canBeDismissed={dialog.canBeDismissed} content={dialog.content} dispatch={dispatch} id={dialog.id} title={dialog.title} />)}
    </div>;
}
...
function ModalDialogImpl({
  ...
  return <div ref={dialogRef} className={styles.Dialog} onClick={handleDialogClick}>
      {title !== null && <div className={styles.Title}>{title}</div>}
      {content}
      {canBeDismissed && <div className={styles.Buttons}>
          <Button autoFocus={true} className={styles.Button} onClick={dismissModal}>
            Okay
          </Button>
        </div>}
    </div>;
