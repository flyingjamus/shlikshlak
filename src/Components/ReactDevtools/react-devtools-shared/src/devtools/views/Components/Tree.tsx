...
export default function Tree(props: Props) {
  ...
  const { numElements, ownerID, searchIndex, searchResults, selectedElementID, selectedElementIndex } =
  ...
  const { lineHeight, showInlineWarningsAndErrors } = { lineHeight: 20, showInlineWarningsAndErrors: true }
  ...
  const itemKey = useCallback((index: number) => store.getElementIDAtIndex(index), [store])
        ...
        <div className={styles.SearchInput}>
          ...
          {showInlineWarningsAndErrors && ownerID === null && (errors > 0 || warnings > 0) && (
            <React.Fragment>
              ...
              <Button onClick={handlePreviousErrorOrWarningClick} title="Scroll to previous error or warning">
                <ButtonIcon type="up" />
              </Button>
              <Button onClick={handleNextErrorOrWarningClick} title="Scroll to next error or warning">
                <ButtonIcon type="down" />
              </Button>
              <Button onClick={clearErrorsAndWarnings} title="Clear all errors and warnings">
...
function updateIndentationSizeVar(
  ...
  const list = innerDiv.parentElement as any as HTMLDivElement
...
function InnerElementType({ children, style, ...rest }) {
  ...
  const cachedChildWidths = useMemo<WeakMap<HTMLElement, number>>(() => new WeakMap(), [])
  ...
  return (
    <div className={styles.InnerElementType} ref={divRef} style={style} {...rest}>
