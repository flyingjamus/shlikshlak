...
export default function Element({ data, index, style }: Props) {
  ...
  const { ownerFlatTree, ownerID, selectedElementID } = useContext(TreeStateContext)
  ...
  const { depth, displayName, hocDisplayNames, isStrictModeNonCompliant, key, type } =
    element as any as ElementType
  ...
  if (isSelected) {
    className = treeFocused ? styles.SelectedElement : styles.InactiveSelectedElement
  ...
  return (
    ...
    >
      ...
      >
        {ownerID === null ? <ExpandCollapseToggle element={element} store={store} /> : null}

        <DisplayName displayName={displayName} id={id as any as number} />
        ...
        {hocDisplayNames !== null && hocDisplayNames.length > 0 ? (
          <Badge className={styles.Badge} hocDisplayNames={hocDisplayNames} type={type}>
            <DisplayName displayName={hocDisplayNames[0]} id={id as any as number} />
          </Badge>
        ) : null}
        {showInlineWarningsAndErrors && errorCount > 0 && (
          <Icon
            type="error"
            className={isSelected && treeFocused ? styles.ErrorIconContrast : styles.ErrorIcon}
          />
        )}
        {showInlineWarningsAndErrors && warningCount > 0 && (
          <Icon
            type="warning"
            className={isSelected && treeFocused ? styles.WarningIconContrast : styles.WarningIcon}
          />
        )}
        {showStrictModeBadge && (
          <Icon
            className={isSelected && treeFocused ? styles.StrictModeContrast : styles.StrictMode}
...
function DisplayName({ displayName, id }: DisplayNameProps) {
  ...
  children.push(
    <mark key="middle" className={isCurrentResult ? styles.CurrentHighlight : styles.Highlight}>
