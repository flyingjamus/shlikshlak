...
import styles from "./NewArrayValue.module.css";
...
export default function NewArrayValue({
  ...
  return <div key={key} hidden={hidden} style={{
    paddingLeft: `${(depth - 1) * 0.75}rem`
  }}>
      <div className={styles.NewArrayValue}>
        <EditableName allowWhiteSpace={true} autoFocus={key > 0} className={[styles.EditableName, isInvalid && styles.Invalid].join(' ')} initialValue="" overrideName={overrideName} path={path} />
      </div>
    </div>;
}
