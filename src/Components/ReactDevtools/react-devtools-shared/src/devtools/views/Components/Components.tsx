...
import { Fragment, Suspense, useEffect, useLayoutEffect, useReducer, useRef } from 'react'
...
function Components(_: {}) {
  ...
  const [state, dispatch] = useReducer<ResizeState, any, ResizeAction>(resizeReducer, null, initResizeState)
  ...
  if (isResizing) {
    ...
    onResize = (event) => {
      ...
      if (isMousePositionInBounds) {
        ...
        const actionType =
          orientation === 'horizontal' ? 'ACTION_SET_HORIZONTAL_PERCENTAGE' : 'ACTION_SET_VERTICAL_PERCENTAGE'
...
function resizeReducer(state: ResizeState, action: ResizeAction): ResizeState {
  switch (action.type) {
    ...
    case 'ACTION_SET_VERTICAL_PERCENTAGE':
      return { ...state, verticalPercentage: action.payload }
...
function getOrientation(wrapperElement: null | HTMLElement): null | Orientation {
