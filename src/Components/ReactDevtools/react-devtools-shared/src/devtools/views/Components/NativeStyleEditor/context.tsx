...
const NativeStyleContext = createContext<Context>(null as any as Context)
...
const resource: Resource<Element, Element, StyleAndLayoutFrontend> = createResource(
  (element: Element) => {
...
function NativeStyleContextController({ children }: Props) {
  ...
  const [currentStyleAndLayout, setCurrentStyleAndLayout] = useState<StyleAndLayoutFrontend | null>(null)
  ...
  return <NativeStyleContext.Provider value={value}>{children}</NativeStyleContext.Provider>
}
