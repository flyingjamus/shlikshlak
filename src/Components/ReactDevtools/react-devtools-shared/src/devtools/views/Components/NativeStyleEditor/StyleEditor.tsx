...
function NewRow({ changeAttribute, changeValue, validAttributes }: NewRowProps) {
  ...
  const changeAttributeWrapper = (oldAttribute: string, newAttribute: string, value: any) => {
...
function Row({
  ...
  const validateAndSetLocalAttribute = (newAttribute) => {
    const isValid =
      newAttribute === '' || validAttributes === null || validAttributes.indexOf(newAttribute) >= 0
    batchedUpdates(() => {
...
function Field({ className, onChange, onReset, onSubmit, placeholder, value }: FieldProps) {
