...
import styles from './InspectedElementView.module.css'
...
export default function InspectedElementView({
  ...
  const { id } = element
  const { owners, rendererPackageName, rendererVersion, rootType, source } = inspectedElement
  ...
  return (
    <Fragment>
      <div className={styles.InspectedElement}>
        ...
        <InspectedElementSuspenseToggle bridge={bridge} inspectedElement={inspectedElement} store={store} />
        ...
        {showRenderedBy && (
          <div className={styles.Owners} data-testname="InspectedElementView-Owners">
            <div className={styles.OwnersHeader}>rendered by</div>
            {showOwnersList &&
              (owners as any as Array<SerializedElement>).map((owner) => (
                <OwnerView
                  key={owner.id}
            ...
            {rootType !== null && <div className={styles.OwnersMetaField}>{rootType}</div>}
            {rendererLabel !== null && <div className={styles.OwnersMetaField}>{rendererLabel}</div>}
          </div>
        )}

        {source !== null && <Source fileName={source.fileName} lineNumber={source.lineNumber} />}
      </div>
      ...
      {isContextMenuEnabledForInspectedElement && (
        <ContextMenu id="InspectedElement">
          {({ path, type: pathType }) => {
            ...
            return (
              <Fragment>
                <ContextMenuItem onClick={copyInspectedElementPath} title="Copy value to clipboard">
                  <Icon className={styles.ContextMenuIcon} type="copy" /> Copy value to clipboard
                </ContextMenuItem>
                <ContextMenuItem onClick={storeAsGlobal} title="Store as global variable">
                  <Icon className={styles.ContextMenuIcon} type="store-as-global-variable" /> Store as global
                  variable
                </ContextMenuItem>
                {viewAttributeSourceFunction !== null && pathType === 'function' && (
                  <ContextMenuItem
                    onClick={() => viewAttributeSourceFunction(id, path)}
                    title="Go to definition"
                  >
                    <Icon className={styles.ContextMenuIcon} type="code" /> Go to definition
                  </ContextMenuItem>
                )}
              </Fragment>
            )
...
function Source({ fileName, lineNumber }: SourceProps) {
  ...
  return (
    <div className={styles.Source} data-testname="InspectedElementView-Source">
      ...
      <div className={styles.SourceOneLiner}>{formatSourceForDisplay(fileName, lineNumber)}</div>
    </div>
  )
}
...
function OwnerView({ displayName, hocDisplayNames, id, isInStore, type }: OwnerViewProps) {
  ...
  return (
    ...
    >
      <span className={styles.OwnerContent}>
        <span className={`${styles.Owner} ${isInStore ? '' : styles.NotInStore}`} title={displayName}>
          {displayName}
        </span>
