...
import styles from './InspectedElementHooksTree.module.css'
import useContextMenu from '../../ContextMenu/useContextMenu'
import { meta } from '../../../hydration'
import { getHookSourceLocationKey } from '../../../hookNamesCache'
...
export function InspectedElementHooksTree({
  ...
  if (hooks === null) {
    ...
    return (
      <div className={styles.HooksTreeView} data-testname="InspectedElementHooksTree">
...
function HookView({ element, hook, hookNames, id, inspectedElement, path }: HookViewProps) {
  const { canEditHooks, canEditHooksAndDeletePaths, canEditHooksAndRenamePaths } = inspectedElement
  ...
  useContextMenu({
    data: {
      path: ['hooks', ...path],
      type:
        hook !== null && typeof hook === 'object' && hook.hasOwnProperty(meta.type)
          ? hook[meta.type as any]
          : typeof value,
    },
    id: 'InspectedElement',
  ...
  const hookName =
    hookNames != null && hookSource != null ? hookNames.get(getHookSourceLocationKey(hookSource)) : null
      ...
      return (
        <div className={styles.Hook}>
          <div ref={contextMenuTriggerRef} className={styles.NameValueRow}>
            ...
            <span onClick={toggleIsOpen} className={name !== '' ? styles.Name : styles.NameAnonymous}>
