...
import styles from './InspectedElement.module.css'
import type { InspectedElement } from './types'
export type Props = {} // TODO Make edits and deletes also use transition API!

export default function InspectedElementWrapper(_: Props) {
  const { inspectedElementID } = useContext(TreeStateContext)
  const dispatch = useContext(TreeDispatcherContext)
  const { canViewElementSourceFunction, viewElementSourceFunction } = useContext(ViewElementSourceContext)
  ...
  const viewSource = useCallback(() => {
    if (viewElementSourceFunction != null && inspectedElement !== null) {
      viewElementSourceFunction(inspectedElement.id, inspectedElement as any as InspectedElement)
    }
  ...
  const canViewSource =
    ...
    (canViewElementSourceFunction === null || canViewElementSourceFunction(inspectedElement))
    ...
    <div className={styles.InspectedElement}>
      <div className={styles.TitleRow} data-testname="InspectedElement-Title">
        ...
        <div className={styles.SelectedComponentName}>
          <div
            className={
              element.isStrictModeNonCompliant ? styles.StrictModeNonCompliantComponent : styles.Component
            }
        ...
        {canOpenInEditor && (
          <Button className={styles.IconButton} onClick={onOpenInEditor} title="Open in editor">
        ...
        {canToggleError && (
          <Toggle
            ...
            title={
              isErrored ? 'Clear the forced error' : 'Force the selected component into an errored state'
            }
          >
      ...
      {inspectedElement === null && <div className={styles.Loading}>Loading...</div>}
