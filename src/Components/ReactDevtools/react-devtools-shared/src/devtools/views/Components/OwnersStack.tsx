...
import { Fragment, useCallback, useContext, useLayoutEffect, useReducer, useRef, useState } from "react";
...
import { Menu, MenuList, MenuButton, MenuItem } from "../Components/reach-ui/menu-button";
import type { SerializedElement } from "./types";
import styles from "./OwnersStack.module.css";
...
export default function OwnerStack() {
  ...
  const selectOwner = useCallback<SelectOwner>((owner: SerializedElement | null) => {
    if (owner !== null) {
  ...
  return <div className={styles.OwnerStack}>
      <div className={styles.Bar} ref={elementsBarRef}>
        {isOverflowing && <Fragment>
            <ElementsDropdown owners={owners} selectedIndex={selectedIndex} selectOwner={selectOwner} />
            <BackToOwnerButton owners={owners} selectedIndex={selectedIndex} selectOwner={selectOwner} />
            {selectedOwner != null && <ElementView owner={selectedOwner} isSelected={true} selectOwner={selectOwner} />}
          </Fragment>}
        {!isOverflowing && owners.map((owner, index) => <ElementView key={index} owner={owner} isSelected={index === selectedIndex} selectOwner={selectOwner} />)}
      </div>
      <div className={styles.VRule} />
      <Button className={styles.IconButton} onClick={() => selectOwner(null)} title="Back to tree view">
...
function ElementsDropdown({
  ...
  for (let index = owners.length - 1; index >= 0; index--) {
    ...
    menuItems.push(<MenuItem key={owner.id} className={`${styles.Component} ${isInStore ? '' : styles.NotInStore}`} onSelect={() => isInStore ? selectOwner(owner) : null}>
        {owner.displayName}

        <Badge className={styles.Badge} hocDisplayNames={owner.hocDisplayNames} type={owner.type} />
      </MenuItem>);
  }
...
function ElementView({
  ...
  return <Toggle className={`${styles.Component} ${isInStore ? '' : styles.NotInStore}`} isChecked={isSelected} onChange={handleChange}>
      {displayName}

      <Badge className={styles.Badge} hocDisplayNames={hocDisplayNames} type={type} />
    </Toggle>;
}
...
function BackToOwnerButton({
  ...
  const isInStore = store.containsElement(owner.id);
  return <Button className={isInStore ? undefined : styles.NotInStore} onClick={() => isInStore ? selectOwner(owner) : null} title={`Up to ${owner.displayName || 'owner'}`}>
