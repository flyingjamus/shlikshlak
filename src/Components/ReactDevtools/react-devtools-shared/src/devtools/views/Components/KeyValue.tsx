...
export default function KeyValue({
  ...
  useContextMenu({
    data: {
      path: [pathRoot, ...path],
      type: value !== null && typeof value === 'object' && hasOwnProperty.call(value, meta.type) ? value[meta.type] : typeof value
    },
    id: 'InspectedElement',
  ...
  if (isDirectChildOfAnArray) {
    if (canDeletePaths) {
      renderedName = <DeleteToggle name={name} deletePath={deletePath} path={path} />;
    } else {
      renderedName = <span className={styles.Name}>
          {name}
          {!!hookName && <span className={styles.HookName}>({hookName})</span>}
        </span>;
    }
  } else if (canRenameTheCurrentPath) {
    renderedName = <EditableName allowEmpty={canDeletePaths} className={styles.EditableName} initialValue={name} overrideName={renamePath} path={path} />;
  } else {
    renderedName = <span className={styles.Name} data-testname="NonEditableName">
        {name}
        {!!hookName && <span className={styles.HookName}>({hookName})</span>}
      </span>;
  }
  ...
  if (isSimpleType) {
    ...
    children = <div key="root" className={styles.Item} hidden={hidden} ref={contextMenuTriggerRef} style={style}>
        <div className={styles.ExpandCollapseToggleSpacer} />
        {renderedName}
        <div className={styles.AfterName}>:</div>
        {canEditValues ? <EditableValue overrideValue={overrideValue} path={path} value={value} /> : shouldDisplayValueAsLink ? <a className={styles.Link} href={value} target="_blank" rel="noopener noreferrer">
            {displayValue}
          </a> : <span className={styles.Value} data-testname="NonEditableValue">
            {displayValue}
          </span>}
      </div>;
  } else if (hasOwnProperty.call(value, meta.type) && !hasOwnProperty.call(value, meta.unserializable)) {
    children = <div key="root" className={styles.Item} hidden={hidden} ref={contextMenuTriggerRef} style={style}>
        {isInspectable ? <ExpandCollapseToggle isOpen={isOpen} setIsOpen={toggleIsOpen} /> : <div className={styles.ExpandCollapseToggleSpacer} />}
        {renderedName}
        <div className={styles.AfterName}>:</div>
        <span className={styles.Value} onClick={isInspectable ? toggleIsOpen : undefined}>
    ...
    if (isArray(value)) {
      ...
      children = value.map((innerValue, index) => <KeyValue key={index} alphaSort={alphaSort} bridge={bridge} canDeletePaths={canDeletePaths && !isReadOnlyBasedOnMetadata} canEditValues={canEditValues && !isReadOnlyBasedOnMetadata} canRenamePaths={canRenamePaths && !isReadOnlyBasedOnMetadata} canRenamePathsAtDepth={canRenamePathsAtDepth} depth={depth + 1} element={element} hookID={hookID} inspectedElement={inspectedElement} isDirectChildOfAnArray={true} hidden={hidden || !isOpen} name={index} path={path.concat(index)} pathRoot={pathRoot} store={store} value={value[index]} />);

      if (canEditValues && !isReadOnlyBasedOnMetadata) {
        children.push(<NewArrayValue key="NewKeyValue" bridge={bridge} depth={depth + 1} hidden={hidden || !isOpen} hookID={hookID} index={value.length} element={element} inspectedElement={inspectedElement} path={path} store={store} type={pathRoot} />);
      }

      children.unshift(<div key={`${depth}-root`} className={styles.Item} hidden={hidden} ref={contextMenuTriggerRef} style={style}>
          {hasChildren ? <ExpandCollapseToggle isOpen={isOpen} setIsOpen={setIsOpen} /> : <div className={styles.ExpandCollapseToggleSpacer} />}
          {renderedName}
          <div className={styles.AfterName}>:</div>
          <span className={styles.Value} onClick={hasChildren ? toggleIsOpen : undefined}>
            {displayName}
          </span>
        </div>);
    } else {
      ...
      const displayName = getMetaValueLabel(value);
      children = entries.map<ReactElement<any>>(([key, keyValue]) => <KeyValue key={key} alphaSort={alphaSort} bridge={bridge} canDeletePaths={canDeletePaths && !isReadOnlyBasedOnMetadata} canEditValues={canEditValues && !isReadOnlyBasedOnMetadata} canRenamePaths={canRenamePaths && !isReadOnlyBasedOnMetadata} canRenamePathsAtDepth={canRenamePathsAtDepth} depth={depth + 1} element={element} hookID={hookID} inspectedElement={inspectedElement} hidden={hidden || !isOpen} name={key} path={path.concat(key)} pathRoot={pathRoot} store={store} value={keyValue} />);

      if (canEditValues && !isReadOnlyBasedOnMetadata) {
        children.push(<NewKeyValue key="NewKeyValue" bridge={bridge} depth={depth + 1} element={element} hidden={hidden || !isOpen} hookID={hookID} inspectedElement={inspectedElement} path={path} store={store} type={pathRoot} />);
      }
...
function DeleteToggle({
  ...
  return <>
      <Button className={styles.DeleteArrayItemButton} onClick={handleClick} title="Delete entry">
