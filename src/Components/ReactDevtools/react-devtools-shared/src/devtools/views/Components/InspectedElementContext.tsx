...
import { checkForUpdate, inspectElement } from '../../../inspectedElementCache'
import { clearHookNamesCache, hasAlreadyLoadedHookNames, loadHookNames } from '../../../hookNamesCache'
import { loadModule } from '../../../dynamicImportCache'
...
export const InspectedElementContext = createContext<Context>(null as any as Context)
...
export function InspectedElementContextController({ children }: Props) {
      ...
      if (typeof hookNamesModuleLoader === 'function') {
        if (parseHookNames || alreadyLoadedHookNames) {
          ...
          if (hookNamesModule !== null) {
            const { parseHookNames: loadHookNamesFunction, purgeCachedMetadata } = hookNamesModule
  ...
  useEffect(() => {
    ...
    element,
    hookNames, // Reset this timer any time the element we're inspecting gets a new response.
  ...
  return <InspectedElementContext.Provider value={value}>{children}</InspectedElementContext.Provider>
}
