...
import { createContext, useCallback, useContext, useEffect, useLayoutEffect, useMemo, useReducer, useRef, startTransition } from "react";
  ...
  if (ownerID === null) {
    switch (action.type) {
      ...
      case 'GO_TO_PREVIOUS_SEARCH_RESULT':
        if (numPrevSearchResults > 0) {
          didRequestSearch = true;
          searchIndex = ((searchIndex as any) as number) > 0 ? ((searchIndex as any) as number) - 1 : numPrevSearchResults - 1;
        }
...
function TreeContextController({
  ...
  const reducer = useMemo(() => (state: State, action: Action): State => {
    const {
      type
    } = action;
  ...
  useEffect(() => {
    const handleSelectFiber = (id: number) => dispatchWrapper({
      type: 'SELECT_ELEMENT_BY_ID',
      payload: id
    });
  ...
  useEffect(() => {
    const handleStoreMutated = ([addedElementIDs, removedElementIDs]: [Array<number>, Map<number, number>]) => {
      dispatchWrapper({
        type: 'HANDLE_STORE_MUTATION',
  ...
  return <TreeStateContext.Provider value={state}>
...
function recursivelySearchTree(store: Store, elementID: number, regExp: RegExp, searchResults: Array<number>): void {
  const {
    children,
    displayName,
    hocDisplayNames
  } = ((store.getElementByID(elementID) as any) as Element);
