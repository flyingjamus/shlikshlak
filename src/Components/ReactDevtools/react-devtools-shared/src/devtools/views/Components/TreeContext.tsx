...
const TreeStateContext = createContext<StateContext>(null as any as StateContext)
TreeStateContext.displayName = 'TreeStateContext'
const TreeDispatcherContext = createContext<DispatcherContext>(null as any as DispatcherContext)
...
function reduceTreeState(store: Store, state: State, action: Action): State {
  let { numElements, ownerSubtreeLeafElementID, selectedElementIndex, selectedElementID } = state
    ...
    switch (action.type) {
      case 'HANDLE_STORE_MUTATION':
        ...
        while (selectedElementID !== null && removedIDs.has(selectedElementID)) {
          selectedElementID = removedIDs.get(selectedElementID) as any as number
        }
      ...
      case 'SELECT_CHILD_ELEMENT_IN_TREE':
        ...
        if (selectedElementIndex !== null) {
          const selectedElement = store.getElementAtIndex(selectedElementIndex as any as number)
      ...
      case 'SELECT_ELEMENT_AT_INDEX':
        ownerSubtreeLeafElementID = null
        selectedElementIndex = (action as ACTION_SELECT_ELEMENT_AT_INDEX).payload
        break
      ...
      case 'SELECT_ELEMENT_BY_ID':
        ...
        selectedElementID = (action as ACTION_SELECT_ELEMENT_BY_ID).payload
        selectedElementIndex =
          selectedElementID === null ? null : store.getIndexOfElementID(selectedElementID)
        break
        ...
        if (selectedElementIndex !== null) {
          ...
          if (selectedElement !== null && selectedElement.parentID !== 0) {
            ...
            if (parent !== null) {
              ...
              const nextChildID =
                selectedChildIndex < children.length - 1 ? children[selectedChildIndex + 1] : children[0]
        ...
        if (selectedElementIndex !== null) {
          ...
          if (selectedElement !== null && selectedElement.parentID !== 0) {
            ...
            if (parent !== null) {
              ...
              const selectedChildIndex = children.indexOf(selectedElement.id)
              const nextChildID =
                selectedChildIndex > 0 ? children[selectedChildIndex - 1] : children[children.length - 1]
              selectedElementIndex = store.getIndexOfElementID(nextChildID)
            }
          }
        }
  ...
  if (lookupIDForIndex && selectedElementIndex !== state.selectedElementIndex) {
    if (selectedElementIndex === null) {
      ...
      selectedElementID = store.getElementIDAtIndex(selectedElementIndex as any as number)
    }
  }

  return { ...state, numElements, ownerSubtreeLeafElementID, selectedElementIndex, selectedElementID }
}

function reduceSearchState(store: Store, state: State, action: Action): State {
  let { searchIndex, searchResults, searchText, selectedElementID, selectedElementIndex } = state
  const ownerID = state.ownerID
    ...
    switch (action.type) {
      ...
      case 'GO_TO_PREVIOUS_SEARCH_RESULT':
        if (numPrevSearchResults > 0) {
          didRequestSearch = true
          searchIndex =
            (searchIndex as any as number) > 0 ? (searchIndex as any as number) - 1 : numPrevSearchResults - 1
        }
      ...
      case 'HANDLE_STORE_MUTATION':
        if (searchText !== '') {
          const [addedElementIDs, removedElementIDs] = (action as ACTION_HANDLE_STORE_MUTATION).payload
          removedElementIDs.forEach((parentID, id) => {
            ...
            if (index >= 0) {
              ...
              if (searchResults.length === 0) {
                searchIndex = null
              } else if ((searchIndex as any as number) >= searchResults.length) {
          ...
          addedElementIDs.forEach((id) => {
            ...
            if (element !== null) {
              ...
              if (displayName !== null && regExp.test(displayName)) {
                const newElementIndex = store.getIndexOfElementID(id) as any as number
                let foundMatch = false

                for (let index = 0; index < searchResults.length; index++) {
                  const resultID = searchResults[index]

                  if (newElementIndex < (store.getIndexOfElementID(resultID) as any as number)) {
                    foundMatch = true
        ...
        if (searchText !== '') {
          ...
          if (searchResults.length > 0) {
            if (prevSearchIndex === null) {
              ...
              searchIndex = Math.min(prevSearchIndex as any as number, searchResults.length - 1)
            }
          }
        }
  ...
  if (didRequestSearch && searchIndex !== null) {
    selectedElementID = searchResults[searchIndex] as any as number
    selectedElementIndex = store.getIndexOfElementID(selectedElementID as any as number)
  }

  return { ...state, selectedElementID, selectedElementIndex, searchIndex, searchResults, searchText }
}

function reduceOwnersState(store: Store, state: State, action: Action): State {
  let { numElements, selectedElementID, selectedElementIndex, ownerID, ownerFlatTree } = state
  ...
  switch (type) {
    case 'UPDATE_INSPECTED_ELEMENT_ID':
      if (state.inspectedElementID !== state.selectedElementID) {
        return { ...state, inspectedElementID: state.selectedElementID }
      }
...
type Props = {
  ...
  defaultInspectedElementID?: number | null | undefined
  defaultOwnerID?: number | null | undefined
  defaultSelectedElementID?: number | null | undefined
  defaultSelectedElementIndex?: number | null | undefined
}
...
function TreeContextController({
        ...
        switch (type) {
          ...
          case 'SET_SEARCH_TEXT':
            ...
            if (state.selectedElementID !== null && store.isInsideCollapsedSubTree(state.selectedElementID)) {
              return { ...state, selectedElementIndex: null }
            }
  ...
  return (
    <TreeStateContext.Provider value={state}>
      <TreeDispatcherContext.Provider value={dispatchWrapper}>{children}</TreeDispatcherContext.Provider>
...
function recursivelySearchTree(
  ...
  const { children, displayName, hocDisplayNames } = store.getElementByID(elementID) as any as Element
