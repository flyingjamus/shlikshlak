...
import { separateDisplayNameAndHOCs } from '../../../utils';
import type { Resource, Thenable } from "../../cache";
import { OwnersList, SerializedElement } from './types'
type Context = (id: number) => Array<SerializedElement> | null;
const OwnersListContext = createContext<Context>(((null as any) as Context));
...
const resource: Resource<Element, Element, Array<SerializedElement>> = createResource((element: Element) => {
  ...
  let resolveFn = ((null as any) as ResolveFn);
...
function OwnersListContextController({
      ...
      if (element !== null) {
        ...
        if (request != null) {
          ...
          request.resolveFn(ownersList.owners === null ? null : ownersList.owners.map(owner => {
            const [displayNameWithoutHOCs, hocDisplayNames] = separateDisplayNameAndHOCs(owner.displayName, owner.type);
            return { ...owner,
  ...
  return <OwnersListContext.Provider value={read}>
      {children}
