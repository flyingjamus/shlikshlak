...
export default function portaledContent(Component: React.FC<any>): React.FC<any> {
  return function PortaledContent({
    portalContainer,
    ...rest
  }: Props) {
    const store = useContext(StoreContext);
    let children = <ErrorBoundary store={store}>
        <Component {...rest} />
      </ErrorBoundary>;
    ...
    if (portalContainer != null) {
      ...
      children = <ThemeProvider>
          <div data-react-devtools-portal-root={true} style={{
          width: '100vw',
          height: '100vh'
        }}>
            {children}
