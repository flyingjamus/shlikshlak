...
export function alphaSortEntries(entryA: [string, unknown], entryB: [string, unknown]): number {
  const a = entryA[0]
  const b = entryB[0]
...
export function getMetaValueLabel(data: Record<string, any>): string | null {
...
function sanitize(data: Record<string, any>): void {
...
export function serializeDataForCopy(props: Record<string, any>): string {
...
export function serializeHooksForCopy(hooks: any | null): string {
...
export function downloadFile(element: HTMLAnchorElement, filename: string, text: string): void {
