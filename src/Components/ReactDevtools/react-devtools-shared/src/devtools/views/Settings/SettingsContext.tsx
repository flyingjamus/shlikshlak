...
const SettingsContext = createContext<Context>(null as any as Context)
...
function SettingsContextController({
  ...
  const [theme, setTheme] = useLocalStorageWithLog<Theme>('React::DevTools::theme', 'auto')
  const [appendComponentStack, setAppendComponentStack] = useLocalStorageWithLog<boolean>(
    LOCAL_STORAGE_SHOULD_APPEND_COMPONENT_STACK_KEY,
    true
  )
  const [breakOnConsoleErrors, setBreakOnConsoleErrors] = useLocalStorageWithLog<boolean>(
    LOCAL_STORAGE_SHOULD_BREAK_ON_CONSOLE_ERRORS,
    false
  )
  ...
  const [hideConsoleLogsInStrictMode, sethideConsoleLogsInStrictMode] = useLocalStorageWithLog<boolean>(
    LOCAL_STORAGE_HIDE_CONSOLE_LOGS_IN_STRICT_MODE,
    false
  )
  const [showInlineWarningsAndErrors, setShowInlineWarningsAndErrors] = useLocalStorageWithLog<boolean>(
    LOCAL_STORAGE_SHOW_INLINE_WARNINGS_AND_ERRORS_KEY,
    true
  )
  const [traceUpdatesEnabled, setTraceUpdatesEnabled] = useLocalStorageWithLog<boolean>(
    LOCAL_STORAGE_TRACE_UPDATES_ENABLED_KEY,
    false
  )
  const documentElements = useMemo<DocumentElements>(() => {
    const array: Array<HTMLElement> = [document.documentElement as any as HTMLElement]

    if (componentsPortalContainer != null) {
      array.push(componentsPortalContainer.ownerDocument.documentElement as any as HTMLElement)
    }

    if (profilerPortalContainer != null) {
      array.push(profilerPortalContainer.ownerDocument.documentElement as any as HTMLElement)
    }
...
export function updateThemeVariables(theme: Theme, documentElements: DocumentElements): void {
