...
import styles from "./SnapshotCommitList.module.css";
...
export default function SnapshotCommitList({
  ...
  return <AutoSizer>
      ...
      height,
      width
    }) => <List commitData={commitData} commitTimes={commitTimes} height={height} filteredCommitIndices={filteredCommitIndices} selectedCommitIndex={selectedCommitIndex} selectedFilteredCommitIndex={selectedFilteredCommitIndex} selectCommitIndex={selectCommitIndex} totalDurations={totalDurations} width={width} />}
    </AutoSizer>;
}
type ListProps = {
  commitData: CommitDataFrontend;
...
function List({
  ...
  const itemSize = useMemo(() => Math.max(minBarWidth, width / filteredCommitIndices.length), [filteredCommitIndices, width]);
  const maxDuration = useMemo(() => totalDurations.reduce((max, duration) => Math.max(max, duration), 0), [totalDurations]);
  ...
  if (hoveredCommitIndex !== null) {
    ...
    if (effectDuration !== null || passiveEffectDuration !== null || priorityLevel !== null) {
      tooltipLabel = <ul className={styles.TooltipList}>
          {priorityLevel !== null && <li className={styles.TooltipListItem}>
          ...
          <li className={styles.TooltipListItem}>
            <div className={styles.DurationsWrapper}>
              ...
              <ul className={styles.DurationsList}>
                ...
                {effectDuration !== null && <li className={styles.DurationsListItem}>
                ...
                {passiveEffectDuration !== null && <li className={styles.DurationsListItem}>
                    <label className={styles.DurationsLabel}>
                      Passive effects
                    </label>
                    <span className={styles.DurationsValue}>
  ...
  return <Tooltip className={styles.Tooltip} label={tooltipLabel}>
      <div ref={divRef} style={{
      height,
      width
    }} onMouseLeave={() => setHoveredCommitIndex(null)}>
        <FixedSizeList className={styles.List} layout="horizontal" height={height} itemCount={filteredCommitIndices.length} itemData={itemData} itemSize={itemSize} ref={(listRef as any)} width={width}>
