...
import { formatTimestamp, getSchedulingEventLabel } from "react-devtools-timeline/src/utils/formatting";
import { stackToComponentSources } from "react-devtools-shared/src/devtools/utils";
import { copy } from "clipboard-js";
import styles from "./SidebarEventInfo.module.css";
export type Props = {};
...
function SchedulingEventInfo({
  eventInfo
}: SchedulingEventProps) {
  const {
    viewUrlSourceFunction
  } = useContext(ViewSourceContext);
  const {
    componentName,
    timestamp
  } = eventInfo;
  ...
  return <>
      ...
      <div className={styles.Content} tabIndex={0}>
        <ul className={styles.List}>
          ...
          {componentStack && <li className={styles.ListItem}>
              <div className={styles.Row}>
                <label className={styles.Label}>Rendered by</label>
                <Button onClick={() => copy(componentStack)} title="Copy component stack to clipboard">
              ...
              return <li key={index}>
                        <Button className={source ? styles.ClickableSource : styles.UnclickableSource} disabled={!source} onClick={() => viewSource(source)}>
                          {displayName}
                        </Button>
                      </li>;
            })}
