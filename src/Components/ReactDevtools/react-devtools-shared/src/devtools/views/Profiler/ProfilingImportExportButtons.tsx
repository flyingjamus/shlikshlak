...
import { prepareProfilingDataExport, prepareProfilingDataFrontendFromExport } from "./utils";
import { downloadFile } from "../utils";
import { TimelineContext } from "react-devtools-timeline/src/TimelineContext";
import isArray from "shared/isArray";
import hasOwnProperty from "shared/hasOwnProperty";
import styles from "./ProfilingImportExportButtons.module.css";
...
export default function ProfilingImportExportButtons() {
          ...
          try {
            ...
            modalDialogDispatch({
              ...
              content: <Fragment>
                  ...
                  {error !== null && <div className={styles.ErrorMessage}>{error.message}</div>}
                </Fragment>
            });
          }
        } else {
  ...
  return <Fragment>
      ...
      <input ref={inputRef} className={styles.Input} type="file" accept=".json" onChange={handleChange} tabIndex={-1} />
      <a ref={downloadRef} className={styles.Input} />
      <Button disabled={isProfiling} onClick={clickInputElement} title="Load profile...">
        <ButtonIcon type="import" />
      </Button>
      <Button disabled={isProfiling || !doesHaveInMemoryData} onClick={downloadData} title="Save profile...">
