...
import styles from "./CommitRanked.module.css";
  ...
  if (commitTree != null && chartData != null && chartData.nodes.length > 0) {
    return <div className={styles.Container} onClick={deselectCurrentFiber}>
        <AutoSizer>
          ...
          height,
          width
        }) => <CommitRanked chartData={((chartData as any) as ChartData)} commitTree={((commitTree as any) as CommitTree)} height={height} width={width} />}
        </AutoSizer>
      </div>;
  } else {
    return <NoCommitData />;
...
function CommitRanked({
  ...
  const selectedFiberIndex = useMemo(() => getNodeIndex(chartData, selectedFiberID), [chartData, selectedFiberID]);
  ...
  return <Tooltip label={tooltipLabel}>
      <FixedSizeList height={height} innerElementType="svg" itemCount={chartData.nodes.length} itemData={itemData} itemSize={lineHeight} width={width}>
