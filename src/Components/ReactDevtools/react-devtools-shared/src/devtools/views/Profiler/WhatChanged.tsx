...
export default function WhatChanged({
  ...
  const {
    changeDescriptions
  } = profilerStore.getCommitData(((rootID as any) as number), selectedCommitIndex);
  ...
  if (context === true) {
    changes.push(<div key="context" className={styles.Item}>
        • Context changed
      </div>);
  } else if (typeof context === 'object' && context !== null && context.length !== 0) {
    changes.push(<div key="context" className={styles.Item}>
        • Context changed:
        {context.map(key => <span key={key} className={styles.Key}>
            {key}
          </span>)}
      </div>);
  }
  ...
  if (didHooksChange) {
    if (enableProfilerChangedHookIndices && Array.isArray(hooks)) {
      changes.push(<div key="hooks" className={styles.Item}>
  ...
  if (props !== null && props.length !== 0) {
    changes.push(<div key="props" className={styles.Item}>
        • Props changed:
        {props.map(key => <span key={key} className={styles.Key}>
            {key}
          </span>)}
      </div>);
  }
  ...
  if (state !== null && state.length !== 0) {
    changes.push(<div key="state" className={styles.Item}>
        • State changed:
        {state.map(key => <span key={key} className={styles.Key}>
            {key}
          </span>)}
      </div>);
  }
  ...
  if (changes.length === 0) {
    changes.push(<div key="nothing" className={styles.Item}>
