...
import { TreeDispatcherContext, TreeStateContext } from "../Components/TreeContext";
...
const ProfilerContext = createContext<Context>(((null as any) as Context));
...
function ProfilerContextController({
  ...
  const [prevProfilingData, setPrevProfilingData] = useState<ProfilingDataFrontend | null>(null);
  ...
  const selectFiber = useCallback((id: number | null, name: string | null) => {
    selectFiberID(id);
  ...
  const setRootIDAndClearFiber = useCallback((id: number | null) => {
  ...
  const [isCommitFilterEnabled, setIsCommitFilterEnabled] = useLocalStorage<boolean>('React::DevTools::isCommitFilterEnabled', false);
  const [minCommitDuration, setMinCommitDuration] = useLocalStorage<number>('minCommitDuration', 0);
  const [selectedCommitIndex, selectCommitIndex] = useState<number | null>(null);
  const [selectedTabID, selectTab] = useLocalStorage<TabID>('React::DevTools::Profiler::defaultTab', 'flame-chart', value => {
    logEvent({
  ...
  const stopProfiling = useCallback(() => store.profilerStore.stopProfiling(), [store]);
  ...
  return <ProfilerContext.Provider value={value}>
      {children}
