import {
  ...
  TREE_OPERATION_UPDATE_TREE_BASE,
  TREE_OPERATION_UPDATE_ERRORS_OR_WARNINGS_DURATION,
  TREE_OPERATION_SET_SUBTREE_MODE,
} from '../../../constants'
import { utfDecodeString } from '../../../utils'
import ProfilerStore from '../../ProfilerStore'
import { CommitTree, CommitTreeNode, ProfilingDataForRootFrontend } from './types'
import { ElementType, ElementTypeRoot } from '../../../types'
...
export function getCommitTree({
  ...
  const commitTrees = rootToCommitTreeMap.get(rootID) as any as Array<CommitTree>
...
function recursivelyInitializeTree(
  ...
  if (node != null) {
    nodes.set(id, {
      ...
      key: node.key,
      parentID,
      treeBaseDuration: dataForRoot.initialTreeBaseDurations.get(id) as any as number,
      type: node.type,
    })
...
function updateTree(commitTree: CommitTree, operations: Array<number>): CommitTree {
  ...
  const getClonedNode = (id: number): CommitTreeNode => {
    const clonedNode = Object.assign({}, nodes.get(id)) as any as CommitTreeNode
  ...
  while (i < operations.length) {
    ...
    switch (operation) {
      ...
      case TREE_OPERATION_REORDER_CHILDREN: {
        ...
        const children = operations.slice(i + 3, i + 3 + numChildren) as any as Array<number>
        i = i + 3 + numChildren
...
const __printTree = (commitTree: CommitTree) => {
  if (__DEBUG__) {
    ...
    while (queue.length > 0) {
      ...
      console.log(
        `${'•'.repeat(depth)}${node.id}:${node.displayName || ''} ${node.key ? `key:"${node.key}"` : ''} (${
