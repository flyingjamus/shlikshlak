...
import styles from "./CommitFlamegraph.module.css";
...
export default function CommitFlamegraphAutoSizer(_: {}) {
  ...
  const deselectCurrentFiber = useCallback(event => {
  ...
  if (commitTree != null && chartData != null && chartData.depth > 0) {
    return <div className={styles.Container} onClick={deselectCurrentFiber}>
        <AutoSizer>
          {({
          height,
          width
        ...
        <CommitFlamegraph chartData={((chartData as any) as ChartData)} commitTree={((commitTree as any) as CommitTree)} height={height} width={width} />}
        </AutoSizer>
      </div>;
  } else {
    return <NoCommitData />;
...
function CommitFlamegraph({
  ...
  const tooltipLabel = useMemo(() => hoveredFiberData !== null ? <HoveredFiberInfo fiberData={hoveredFiberData} /> : null, [hoveredFiberData]);
  return <Tooltip label={tooltipLabel}>
      <FixedSizeList height={height} innerElementType={InnerElementType} itemCount={chartData.depth} itemData={itemData} itemSize={lineHeight} width={width}>
...
const InnerElementType = forwardRef(({
  children,
  ...rest
}, ref) => <svg ref={ref} {...rest}>
    <defs>
      <pattern id="didNotRenderPattern" patternUnits="userSpaceOnUse" width="4" height="4">
        <path d="M-1,1 l2,-2 M0,4 l4,-4 M3,5 l2,-2" className={styles.PatternPath} />
      </pattern>
    </defs>
    {children}
  </svg>);
