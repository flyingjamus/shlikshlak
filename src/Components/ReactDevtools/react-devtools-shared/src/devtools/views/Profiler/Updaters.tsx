...
import styles from "./Updaters.module.css";
...
export default function Updaters({
  ...
  const children = updaters.length > 0 ? updaters.map<React.ReactNode>((serializedElement: SerializedElement) => {
    const {
      displayName,
      id,
      key,
    ...
    if (isVisibleInTree) {
      return <button key={id} className={styles.Updater} onClick={() => selectFiber(id, displayName)}>
              {displayName} {key ? `key="${key}"` : ''}
            </button>;
    } else {
      return <div key={id} className={styles.UnmountedUpdater}>
              {displayName} {key ? `key="${key}"` : ''}
            </div>;
    }
