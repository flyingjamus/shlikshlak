...
import styles from "./SidebarSelectedFiberInfo.module.css";
export type Props = {};
export default function SidebarSelectedFiberInfo(_: Props) {
  ...
  const commitIndices = profilingCache.getFiberCommits({
    fiberID: ((selectedFiberID as any) as number),
  ...
  for (i = 0; i < commitIndices.length; i++) {
    ...
    listItems.push(<button key={commitIndex} ref={selectedCommitIndex === commitIndex ? selectedListItemRef : null} className={selectedCommitIndex === commitIndex ? styles.CurrentCommit : styles.Commit} onClick={() => selectCommitIndex(commitIndex)}>
