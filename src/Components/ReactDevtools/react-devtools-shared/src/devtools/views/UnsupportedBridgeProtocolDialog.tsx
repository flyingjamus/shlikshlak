...
import styles from "./UnsupportedBridgeProtocolDialog.module.css";
import { BridgeProtocol, currentBridgeProtocol } from '../../bridge'
      ...
      if (!isVisible) {
        if (store.unsupportedBridgeProtocolDetected) {
          dispatch({
            ...
            id: MODAL_DIALOG_ID,
            type: 'SHOW',
            content: <DialogContent unsupportedBridgeProtocol={store.bridgeProtocol} />
          });
        }
      } else {
        if (!store.unsupportedBridgeProtocolDetected) {
  ...
  if (maxNpmVersion === null) {
    ...
    instructions = <>
        ...
        <pre className={styles.NpmCommand}>
          {upgradeInstructions}
          {/*<Button onClick={() => copy(upgradeInstructions)} title="Copy upgrade command to clipboard">*/}
    ...
    instructions = <>
        ...
        <pre className={styles.NpmCommand}>
          ...
          {/*<Button onClick={() => copy(downgradeInstructions)} title="Copy downgrade command to clipboard">*/}
          {/*  <ButtonIcon type="copy" />*/}
          {/*</Button>*/}
        </pre>
      </>;
