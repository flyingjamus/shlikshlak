...
import { getCommitTree, invalidateCommitTrees } from '../../src/devtools/views/Profiler/CommitTreeBuilder'
import {
...
export default class ProfilingCache {
  ...
  getCommitTree = ({ commitIndex, rootID }: { commitIndex: number; rootID: number }) =>
    getCommitTree({
      commitIndex,
      profilerStore: this._profilerStore,
      rootID,
    })
  getFiberCommits = ({ fiberID, rootID }: { fiberID: number; rootID: number }): Array<number> => {
