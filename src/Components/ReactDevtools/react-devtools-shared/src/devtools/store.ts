...
export default class Store extends EventEmitter<{
  ...
  constructor(bridge: FrontendBridge, config?: Config) {
    ...
    bridge.addListener('isBackendStorageAPISupported', this.onBackendStorageAPISupported)
    bridge.addListener('isNativeStyleEditorSupported', this.onBridgeNativeStyleEditorSupported)
    bridge.addListener('isSynchronousXHRSupported', this.onBridgeSynchronousXHRSupported)
  ...
  get supportsReloadAndProfile(): boolean {
    ...
    return (
      this._supportsReloadAndProfile && this._isBackendStorageAPISupported && this._isSynchronousXHRSupported
    )
  }
    ...
    if (element !== null) {
      if (isCollapsed) {
        ...
        if (!element.isCollapsed) {
          ...
          let parentElement = this._idToElement.get(element.parentID) as any as Element
        ...
        while (currentElement != null) {
          ...
          if (currentElement.isCollapsed) {
            ...
            let parentElement = this._idToElement.get(currentElement.parentID) as any as Element
  ...
  onBridgeOperations = (operations: Array<number>) => {
    ...
    while (i < operations.length) {
      ...
      switch (operation) {
        ...
        default:
          this._throwAndEmitError(
            new UnsupportedBridgeOperationError(`Unsupported Bridge operation "${operation}"`)
          )
      }
    }
  ...
  onBridgeOverrideComponentFilters = (componentFilters: Array<ComponentFilter>) => {
