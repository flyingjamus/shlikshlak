...
const contextStackCursor: StackCursor<Record<string, any>> = createCursor(emptyContextObject);
// A cursor to a boolean indicating whether the context has changed.
const didPerformWorkStackCursor: StackCursor<boolean> = createCursor(false);
// Keep track of the previous context object that was on the stack.
// We use this to get access to the parent context after we have already
// pushed the next context provider, and now need to merge their contexts.
let previousContext: Record<string, any> = emptyContextObject;

function getUnmaskedContext(workInProgress: Fiber, Component: (...args: Array<any>) => any, didPushOwnContextIfProvider: boolean): Record<string, any> {
...
function cacheContext(workInProgress: Fiber, unmaskedContext: Record<string, any>, maskedContext: Record<string, any>): void {
  if (disableLegacyContext) {
    return;
  } else {
...
function getMaskedContext(workInProgress: Fiber, unmaskedContext: Record<string, any>): Record<string, any> {
  if (disableLegacyContext) {
    return emptyContextObject;
  } else {
...
function isContextProvider(type: (...args: Array<any>) => any): boolean {
  if (disableLegacyContext) {
    return false;
  } else {
...
function pushTopLevelContextObject(fiber: Fiber, context: Record<string, any>, didChange: boolean): void {
  if (disableLegacyContext) {
    return;
  } else {
...
function processChildContext(fiber: Fiber, type: any, parentContext: Record<string, any>): Record<string, any> {
  if (disableLegacyContext) {
...
function findCurrentUnmaskedContext(fiber: Fiber): Record<string, any> {
  if (disableLegacyContext) {
    return emptyContextObject;
  } else {
