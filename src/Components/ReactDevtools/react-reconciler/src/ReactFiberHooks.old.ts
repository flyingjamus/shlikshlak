import type { MutableSource, MutableSourceGetSnapshotFn, MutableSourceSubscribeFn, ReactContext, StartTransitionOptions, Usable, Thenable } from "shared/ReactTypes";
...
import { enableDebugTracing, enableSchedulingProfiler, enableNewReconciler, enableCache, enableUseRefAccessWarning, enableLazyContextPropagation, enableUseMutableSource, enableTransitionTracing, enableUseHook, enableUseMemoCacheHook } from "shared/ReactFeatureFlags";
...
import { LayoutStatic as LayoutStaticEffect, Passive as PassiveEffect, PassiveStatic as PassiveStaticEffect, StaticMask as StaticMaskEffect, Update as UpdateEffect, StoreConsistency } from "./ReactFiberFlags";
...
import { getWorkInProgressRoot, getWorkInProgressRootRenderLanes, scheduleUpdateOnFiber, requestUpdateLane, requestEventTime, markSkippedUpdateLanes } from "./ReactFiberWorkLoop.old";
...
function use<T>(usable: Usable<T>): T {
  if (usable !== null && typeof usable === 'object' && typeof usable.then === 'function') {
    ...
    switch (thenable.status) {
      ...
      default:
        {
          const prevThenableAtIndex: Thenable<T> | null = getPreviouslyUsedThenableAtIndex(index);
          ...
          if (prevThenableAtIndex !== null) {
            switch (prevThenableAtIndex.status) {
              ...
              case 'rejected':
                {
                  const rejectedError: unknown = prevThenableAtIndex.reason;
                  throw rejectedError;
                }
...
function rerenderTransition(): [boolean, (callback: () => void, options?: StartTransitionOptions) => void] {
...
if (enableUseHook) {
  (ContextOnlyDispatcher as Dispatcher).use = throwInvalidHookError;
}
...
if (enableUseHook) {
  (HooksDispatcherOnMount as Dispatcher).use = use;
}
...
if (enableUseHook) {
  (HooksDispatcherOnUpdate as Dispatcher).use = use;
}
...
if (enableUseHook) {
  (HooksDispatcherOnRerender as Dispatcher).use = use;
}
...
if (__DEV__) {
  ...
  if (enableUseHook) {
    (HooksDispatcherOnMountInDEV as Dispatcher).use = use;
  }

  if (enableUseMemoCacheHook) {
    (HooksDispatcherOnMountInDEV as Dispatcher).useMemoCache = useMemoCache;
  }
  ...
  if (enableUseHook) {
    (HooksDispatcherOnMountWithHookTypesInDEV as Dispatcher).use = use;
  }

  if (enableUseMemoCacheHook) {
    (HooksDispatcherOnMountWithHookTypesInDEV as Dispatcher).useMemoCache = useMemoCache;
  }
  ...
  if (enableUseHook) {
    (HooksDispatcherOnUpdateInDEV as Dispatcher).use = use;
  }

  if (enableUseMemoCacheHook) {
    (HooksDispatcherOnUpdateInDEV as Dispatcher).useMemoCache = useMemoCache;
  }
  ...
  if (enableUseHook) {
    (HooksDispatcherOnRerenderInDEV as Dispatcher).use = use;
  }

  if (enableUseMemoCacheHook) {
    (HooksDispatcherOnRerenderInDEV as Dispatcher).useMemoCache = useMemoCache;
  }
  ...
  if (enableUseHook) {
    (InvalidNestedHooksDispatcherOnMountInDEV as Dispatcher).use = function <T>(usable: Usable<T>): T {
      warnInvalidHookAccess();
      return use(usable);
    };
  }
  ...
  if (enableUseMemoCacheHook) {
    (InvalidNestedHooksDispatcherOnMountInDEV as Dispatcher).useMemoCache = function (size: number): Array<any> {
      warnInvalidHookAccess();
  ...
  if (enableUseHook) {
    (InvalidNestedHooksDispatcherOnUpdateInDEV as Dispatcher).use = function <T>(usable: Usable<T>): T {
      warnInvalidHookAccess();
      return use(usable);
    };
  }
  ...
  if (enableUseMemoCacheHook) {
    (InvalidNestedHooksDispatcherOnUpdateInDEV as Dispatcher).useMemoCache = function (size: number): Array<any> {
      warnInvalidHookAccess();
      return useMemoCache(size);
    };
  }
  ...
  if (enableUseHook) {
    (InvalidNestedHooksDispatcherOnRerenderInDEV as Dispatcher).use = function <T>(usable: Usable<T>): T {
      warnInvalidHookAccess();
      return use(usable);
    };
  }
  ...
  if (enableUseMemoCacheHook) {
    (InvalidNestedHooksDispatcherOnRerenderInDEV as Dispatcher).useMemoCache = function (size: number): Array<any> {
      warnInvalidHookAccess();
      return useMemoCache(size);
    };
  }
