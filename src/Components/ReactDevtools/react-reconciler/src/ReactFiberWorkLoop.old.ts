...
import type { PendingTransitionCallbacks, PendingBoundaries, Transition, TransitionAbort } from "./ReactFiberTracingMarkerComponent.old";
...
import { NoFlags, Incomplete, StoreConsistency, HostEffectMask, ForceClientRender, BeforeMutationMask, MutationMask, LayoutMask, PassiveMask, PlacementDEV } from "./ReactFiberFlags";
...
import { resetWakeableStateAfterEachAttempt, resetThenableStateOnCompletion, trackSuspendedWakeable, suspendedThenableDidResolve, isTrackingSuspendedThenable } from "./ReactFiberWakeable.old";
...
export function addMarkerProgressCallbackToPendingTransition(markerName: string, transitions: Set<Transition>, pendingBoundaries: PendingBoundaries) {
  if (enableTransitionTracing) {
    if (currentPendingTransitionCallbacks === null) {
      currentPendingTransitionCallbacks = {
...
export function addMarkerIncompleteCallbackToPendingTransition(markerName: string, transitions: Set<Transition>, aborts: Array<TransitionAbort>) {
  if (enableTransitionTracing) {
    if (currentPendingTransitionCallbacks === null) {
      currentPendingTransitionCallbacks = {
        transitionStart: null,
...
export function scheduleInitialHydrationOnRoot(root: FiberRoot, lane: Lane, eventTime: number) {
...
function resumeSuspendedUnitOfWork(unitOfWork: Fiber, thrownValue: unknown): void {
...
function commitRoot(root: FiberRoot, recoverableErrors: null | Array<CapturedValue<unknown>>, transitions: Array<Transition> | null) {
...
function commitRootImpl(root: FiberRoot, recoverableErrors: null | Array<CapturedValue<unknown>>, transitions: Array<Transition> | null, renderPriorityLevel: EventPriority) {
  do {
...
function doubleInvokeEffectsInDEV(root: FiberRoot, fiber: Fiber, parentIsInStrictMode: boolean) {
