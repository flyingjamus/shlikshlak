...
export function trackSuspendedWakeable(wakeable: Wakeable) {
  ...
  const thenable: Thenable<unknown> = (wakeable as any);
  ...
  switch (thenable.status) {
    ...
    default:
      {
        const pendingThenable: PendingThenable<unknown> = (thenable as any);
        pendingThenable.status = 'pending';
        pendingThenable.then(fulfilledValue => {
          if (thenable.status === 'pending') {
            const fulfilledThenable: FulfilledThenable<unknown> = (thenable as any);
            fulfilledThenable.status = 'fulfilled';
            fulfilledThenable.value = fulfilledValue;
          }
        }, (error: unknown) => {
          if (thenable.status === 'pending') {
            const rejectedThenable: RejectedThenable<unknown> = (thenable as any);
...
export function getPreviouslyUsedThenableAtIndex<T>(index: number): Thenable<T> | null {
  if (usedThenables !== null) {
