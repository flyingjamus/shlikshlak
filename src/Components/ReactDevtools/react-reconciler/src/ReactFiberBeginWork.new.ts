import type { ReactProviderType, ReactContext, ReactNodeList } from "shared/ReactTypes";
...
import type { OffscreenProps, OffscreenState, OffscreenQueue, OffscreenInstance } from "./ReactFiberOffscreenComponent";
...
import { enableCPUSuspense, enableUseMutableSource } from "shared/ReactFeatureFlags";
...
function deferHiddenOffscreenComponent(current: Fiber | null, workInProgress: Fiber, nextBaseLanes: Lanes, renderLanes: Lanes) {
  const nextState: OffscreenState = {
...
function updateTracingMarkerComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes) {
  if (!enableTransitionTracing) {
    return null;
  }
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    const updateQueue: UpdateQueue<RootState> = (workInProgress.updateQueue as any);
...
function mountHostRootWithoutHydrating(current: Fiber, workInProgress: Fiber, nextChildren: ReactNodeList, renderLanes: Lanes, recoverableError: CapturedValue<unknown>) {
...
function shouldRemainOnFallback(current: null | Fiber, workInProgress: Fiber, renderLanes: Lanes) {
  ...
  return hasSuspenseListContext(suspenseContext, (ForceSuspenseFallback as SuspenseContext));
}
    ...
    if (showFallback) {
      ...
      if (enableTransitionTracing) {
        ...
        if (currentTransitions !== null) {
          ...
          const offscreenQueue: OffscreenQueue | null = (primaryChildFragment.updateQueue as any);
    ...
    if (showFallback) {
      ...
      if (enableTransitionTracing) {
        ...
        if (currentTransitions !== null) {
          const parentMarkerInstances = getMarkerInstances();
          const offscreenQueue: OffscreenQueue | null = (primaryChildFragment.updateQueue as any);
          const currentOffscreenQueue: OffscreenQueue | null = (current.updateQueue as any);
...
function retrySuspenseComponentWithoutHydrating(current: Fiber, workInProgress: Fiber, renderLanes: Lanes, recoverableError: CapturedValue<unknown> | null) {
...
function updateDehydratedSuspenseComponent(current: Fiber, workInProgress: Fiber, didSuspend: boolean, nextProps: any, suspenseInstance: SuspenseInstance, suspenseState: SuspenseState, renderLanes: Lanes): null | Fiber {
  if (!didSuspend) {
...
function scheduleSuspenseWorkOnFiber(fiber: Fiber, renderLanes: Lanes, propagationRoot: Fiber) {
