...
function tryToClaimNextHydratableInstance(fiber: Fiber): void {
  ...
  if (!nextInstance) {
    if (shouldClientRenderOnMismatch(fiber)) {
      warnNonhydratedInstance((hydrationParentFiber as any), fiber);
...
function prepareToHydrateHostInstance(fiber: Fiber, hostContext: HostContext): boolean {
  if (!supportsHydration) {
...
function popHydrationState(fiber: Fiber): boolean {
  ...
  if (enableFloat && isHydrating && isHydratableResource(fiber.type, fiber.memoizedProps)) {
    if (fiber.stateNode === null) {
      if (__DEV__) {
        const rel = fiber.memoizedProps.rel ? `rel="${fiber.memoizedProps.rel}" ` : '';
        const href = fiber.memoizedProps.href ? `href="${fiber.memoizedProps.href}"` : '';
        console.error('A matching Hydratable Resource was not found in the DOM for <%s %s%s>.', fiber.type, rel, href);
      }
