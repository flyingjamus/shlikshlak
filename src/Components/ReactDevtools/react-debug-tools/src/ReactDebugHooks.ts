...
} from '../../react-devtools-shared/shared/ReactTypes'
import type { Fiber, Dispatcher as DispatcherType } from '../../react-reconciler/src/ReactInternalTypes'
import ErrorStackParser from 'error-stack-parser'
import assign from '../../react-devtools-shared/shared/assign'
import ReactSharedInternals from '../../react-devtools-shared/shared/ReactSharedInternals'
import {
  FunctionComponent,
  SimpleMemoComponent,
  ContextProvider,
...
// TODO is this the same as the unstable_useCacheRefresh
export function useCacheRefresh(): () => void {
  const hook = nextHook()
  hookLog.push({
...
function useLayoutEffect(create: () => (() => void) | void, inputs: Array<unknown> | void | null): void {
  nextHook()
  hookLog.push({
...
function useInsertionEffect(create: () => unknown, inputs: Array<unknown> | void | null): void {
  nextHook()
  hookLog.push({
...
function useEffect(create: () => (() => void) | void, inputs: Array<unknown> | void | null): void {
  nextHook()
  hookLog.push({
    primitive: 'Effect',
    stackError: new Error(),
...
function useImperativeHandle<T>(
  ref:
    ...
    | void,
  create: () => T,
  inputs: Array<unknown> | void | null
): void {
  nextHook()
...
function useDebugValue(value: any, formatterFn: ((value: any) => any) | null | undefined) {
  hookLog.push({
...
function useCallback<T>(callback: T, inputs: Array<unknown> | void | null): T {
  const hook = nextHook()
  hookLog.push({
    primitive: 'Callback',
    stackError: new Error(),
...
function useMemo<T>(nextCreate: () => T, inputs: Array<unknown> | void | null): T {
  const hook = nextHook()
...
function buildTree(rootStack, readHookLog, includeHooksSource: boolean): HooksTree {
  const rootChildren = []
  let prevStack = null
...
function inspectHooksOfForwardRef<Props, Ref>(
  renderFunction: (arg0: Props, arg1: Ref) => React.ReactNode,
  props: Props,
  ref: Ref,
