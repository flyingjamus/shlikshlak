...
const debug = (methodName: string, ...args: any) => {
  if (__DEBUG__) {
...
export default class Agent extends EventEmitter<{
  ...
  constructor(bridge: BackendBridge) {
    ...
    bridge.send('isSynchronousXHRSupported', true)
    setupHighlighter(bridge, this)
    // setupTraceUpdates(this)
  }

  get rendererInterfaces(): Record<RendererID, RendererInterface> {
  ...
  getInstanceAndStyle({ id, rendererID }: ElementAndRendererID): InstanceAndStyle | null {
    const renderer = this._rendererInterfaces[rendererID]
  ...
  getBestMatchingRendererInterface(node: Record<string, any>): RendererInterface | null {
    let bestMatch = null

    for (const rendererID in this._rendererInterfaces) {
      const renderer = this._rendererInterfaces[rendererID as any] as any as RendererInterface
  ...
  getIDForNode(node: Record<string, any>): number | null {
  ...
  overrideSuspense = ({ id, rendererID, forceFallback }: OverrideSuspenseParams) => {
    const renderer = this._rendererInterfaces[rendererID]
  ...
  overrideValueAtPath = ({ hookID, id, path, rendererID, type, value }: OverrideValueAtPathParams) => {
    const renderer = this._rendererInterfaces[rendererID]
  ...
  overrideContext = ({ id, path, rendererID, wasForwarded, value }: SetInParams) => {
  ...
  // Temporarily support older standalone front-ends by forwarding the older message types
  // to the new "overrideValueAtPath" command the backend is now listening to.
  overrideHookState = ({ id, hookID, path, rendererID, wasForwarded, value }: OverrideHookParams) => {
    // Don't forward a message that's already been forwarded by the front-end Bridge.
    // We only need to process the override command once!
    if (!wasForwarded) {
      this.overrideValueAtPath({
  ...
  // Temporarily support older standalone front-ends by forwarding the older message types
  // to the new "overrideValueAtPath" command the backend is now listening to.
  overrideProps = ({ id, path, rendererID, wasForwarded, value }: SetInParams) => {
    // Don't forward a message that's already been forwarded by the front-end Bridge.
    // We only need to process the override command once!
    if (!wasForwarded) {
      this.overrideValueAtPath({
  ...
  // Temporarily support older standalone front-ends by forwarding the older message types
  // to the new "overrideValueAtPath" command the backend is now listening to.
  overrideState = ({ id, path, rendererID, wasForwarded, value }: SetInParams) => {
    // Don't forward a message that's already been forwarded by the front-end Bridge.
    // We only need to process the override command once!
    if (!wasForwarded) {
      this.overrideValueAtPath({
  ...
  renamePath = ({ hookID, id, newPath, oldPath, rendererID, type }: RenamePathParams) => {
    const renderer = this._rendererInterfaces[rendererID]
  ...
  selectNode(target: Record<string, any>): void {
